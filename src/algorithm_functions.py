...
def check_purity(labels: npt.NDArray[Any]) -> bool:
    """
...
def classify_data(labels: npt.NDArray[Any]) -> float:
    """
...
def get_potential_splits(x_data: npt.NDArray[Any]) -> dict[int, npt.NDArray[Any]]:
    ...
    for i in range(n_columns):
        ...
        potential_splits[i] = np.empty(shape=len(unique_values - 1))  # Create array for column.

        for j, value in enumerate(unique_values):
            if j > 0:
                previous_value = unique_values[j - 1]
                potential_split = (value + previous_value) / 2
                potential_splits[i][j - 1] = potential_split

    return potential_splits


def split_x_y_data(x_data: npt.NDArray[Any], y_data: npt.NDArray[Any], split_column: int,
                   split_threshold: float) -> tuple[npt.NDArray[Any], npt.NDArray[Any],
...
def split_data(data: npt.NDArray[Any], split_column: int, split_threshold: float) -> \
...
def determine_best_split(x_data: npt.NDArray[Any], criterion: str, rounding_factor: int = 2) -> \
        tuple[int, float]:
    """
